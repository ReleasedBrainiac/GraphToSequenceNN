...
from DatasetHandler.ContentSupport import getType
from DatasetHandler.ContentSupport import isAnyNode, isNone, isNotNone, isList
import numpy as np
...
class GTConverter:
    ...
    def GetNeighbourhoodMatrices(self, tree_graph):
        if isNotNone(tree_graph) and isAnyNode(tree_graph):
    ...
    def GetTensorMatricesFromGraphTree(self, tree_graph):
        if isNotNone(tree_graph) and isAnyNode(tree_graph):
    ...
    def GetDataSet(self, data_pairs):
        if isNotNone(data_pairs) and isList(data_pairs):
            ...
            for _ in range(len(data_pairs)):
                ...
                edge_lists = self.GetTensorMatricesFromGraphTree(semantic)
